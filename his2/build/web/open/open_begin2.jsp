...
<html>
    <head>
        ...
        <%
            ...
            // 自动填充学生信息
            String[] studentID = new String[stu.size()];
            String[] studentname = new String[stu.size()];
            String[] sex = new String[stu.size()];
            int[] age = new int[stu.size()];
            String[] major = new String[stu.size()];
            String[] phone = new String[stu.size()];
            int[] sign = new int[stu.size()];
            i = 0;
            if (stu.size() != 0) {
                for (Student stud : stu) {
                    studentID[i] = stud.getStudentID();
                    out.println(i + "=" + studentID[i]);
                    studentname[i] = stud.getName();
                    sex[i] = stud.getSex();
                    age[i] = stud.getAge();
                    major[i] = stud.getMajor();
                    phone[i] = stud.getPhone();
                    sign[i] = stud.getSign();
                    out.println("sign" + i + "=" + sign[i]);
                    i++;
        ...
        <script type="text/javascript" >
            ...
            function insertRowInTable() {
                ...
                newRow.insertCell().innerHTML = "<input type='text' name='open_number' class='open_number' autocomplete='off'  >";
            ...
            <%   for (int j = 0; j < drug_name.length; j++) {%>   // jsp 语句嵌在script 中写，有for 循环的范围的括号，后面补上
                if (drug_name[i - 1].value === "<%=drug_name[j]%>") {
                    ...
                    var t = 0;
                    open_number1[i - 1].options.length = 0; // 清除select 中原来的option 选项
                    for (var j = 0; j < str2.length; j++)
                    {
                        console.log(str2[j]);
                        if (str2[j] !== "")  // 去除为空的数据。
                        {
                            open_number1[i - 1].options[t] = new Option(str2[j]); //操作select 中的option，插入option 选项
                            t++;
                        }
                    }
            ...
            function auto_stuff_add() {   //材料增加
                ...
                newRow.insertCell().innerHTML = "<input type='button' value='删除' onclick='auto_stuff_delete(this)' >&emsp;<input type='button'  value='添加' onclick='auto_stuff_add()' >";
            }
            ...
            function choose_Student() {
                var studID = document.getElementsByName("studentID");
                //  console.log("1=" + studID[0].value);   // 该语句是在if 判断中，只能执行一个，所以就一个
                var name = document.getElementsByName("student_name");
            ...
            <% for (i = 0; i < studentID.length; i++) {%>
                if (studID[0].value === "<%=studentID[i]%>")
                {
                    name[0].value = '<%=studentname[i]%>';
                    console.log("name[0]" + name[0].value);
                    sex[0].value = '<%=sex[i]%>';
                    age[0].value = '<%=age[i]%>';
                    major[0].value = '<%=major[i]%>';
                    phone[0].value = '<%=phone[i]%>';
                    if (<%=sign[i]%> === 0) {
                        var s = "学生";
                    } else {
                        var s = "教职工";
    ...
    <body bgcolor="CCCFFF" >
        ...
        <form action="his2_Servlet?action=open_deal.do" method="post" >  <!--总的提交-->
            <%                String s = null;
                if (stu.size() == 1) {
            %>
            学号/工号：<input type="text"  class="t3" name="studentID" id="studentID"   value="<%=studentID[0]%>"  required="required" autocomplete="off"  >&emsp;<input type="button" value="查询" onclick="select_ID()" >
            &emsp;名字：<input type="text" name="student_name"  id="student_name"   class="tt"   value="<%=studentname[0]%>"  required="required"  autocomplete="off"  > &emsp;<input type="button"  value="查询" onclick="select_name()" >
            <br>
            <br>
            &emsp;性别：<input type="text" name="sex"  class="tt"  value="<%=sex[0]%>" required="required" >
            &emsp;年龄：<input type="text" name="age"   class="tt"  value="<%=age[0]%>"  required="required">
            &emsp;所在单位：<input type="text" name="major"  class="t2"  value="<%=major[0]%>"  required="required">
            &emsp;联系方式：<input type="text" name="phone"  class="tt"  value="<%=phone[0]%>" required="required">
            <%
                if (sign[0] == 0) {
                    s = "学生";
                }
            ...
            %>
            &emsp;职务：<input type="text" name="sign" class="tt"   value="<%=s%>"  >
            <% } else if (stu.size()
                    > 1) {
            %>
            学号/工号：<input type="text"   class="t3" name="studentID" id="studentID" list="student_ID" placeholder="选择查询结果"   onblur="choose_Student(this)"  required="required"  autocomplete="off"  >&emsp;<input type="button" value="查询" onclick="select_ID()" >
            &emsp;名字：<input type="text" name="student_name" id="student_name"  class="tt"  required="required"  autocomplete="off" > &emsp;<input type="button"  value="查询" onclick="select_name()" >
            <br>
            <br>
            &emsp;性别：<input type="text" name="sex"  class="tt" id="sex"  required="required" >
            &emsp;年龄：<input type="text" name="age"  class="tt" id="age"  required="required">
            &emsp;所在单位：<input type="text" name="major"  class="t2" id="major"  required="required">
            &emsp;联系方式：<input type="text" name="phone"  class="tt" id="phone"  required="required">
            &emsp;职务：<input type="text" name="sign" class="tt"  id="sign"   >
            <%
            } else {
            %>
            学号/工号：<input type="text"   class="t3" name="studentID" id="studentID"   placeholder="未查询到相关信息"   required="required"  autocomplete="off"  >&emsp;<input type="button" value="查询" onclick="select_ID()" >
            &emsp;名字：<input type="text" name="student_name" id="student_name"  class="tt" required="required"  autocomplete="off" > &emsp;<input type="button"  value="查询" onclick="select_name()" >
            <br>
            <br>
            ...
            &emsp;职务：<input type="text" name="sign" class="tt"    >
            <%
                }
            ...
            <table id="table" border="1" >
                ...
                <tr> 
                    ...
                    <td><input type="button" value="删除" onclick="auto_stuff_delete(this)" >&emsp;<input type="button"  value="添加" onclick="auto_stuff_add()" ></td>
                </tr>
            </table>
            <br>
        ...
        <datalist id="student_ID">
            <%    for (i = 0;
                        ...
                        i++) {
            %>
            <option value="<%=studentID[i]%>"><%=studentID[i]%></option>
            <%
                }
            %>
        </datalist>
